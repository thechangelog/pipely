...
pragma optional_param ipv6_backend_opt_in false;
pragma optional_param modify_path_in_upgrade false;
pragma optional_param can_upgrade_in_recv true;
pragma optional_param max_backends 20;
pragma optional_param customer_id "3ft";
C!
W!
##############
...
if (req.url.ext ~ "(?i)^(png|jpe?g)$") {
  ...
  if( req.http.host == "changelog.com" && (table.contains(feeds, req.url.path) || req.url.path ~ "^/feeds/") ) {
...
# this one matches feeds stored in our known feeds table
if (req.http.host == "changelog.com" && table.contains(feeds, req.url.path)) {
  set bereq.url = table.lookup(feeds, req.url.path);
}

# this one matches custom feeds that live at /feeds/*
if (req.http.host == "changelog.com" && req.url.path ~ "^/feeds/(.*)$") {
  set bereq.url = regsub(req.url.path, "^/feeds/(.*)$", "/\1.xml");
}
